# PySwitch v2.4.10
- Features:
    - Added **EFFECT_STATE_DYNAMIC** action: works like EFFECT_STATE, but the effect slot is assigned dynamically per rig via a token embedded in the Kemper rig name (e.g. a rig named `Lead ((ACXR))` assigns slot A to digit position 1, C to position 2, X to position 3 and R to position 4), instead of a config-side table — reorganizing which slot a switch controls only needs a rig rename on the Kemper, no PySwitch redeploy. One or more digit positions can be read together (`digit_position` as a list): the LED then reflects AND logic (on only if all resolved slots are engaged), rendered in a single LED write to avoid flicker. An unrecognized character at a requested position (`-` is the suggested convention) disables the button for that rig; a missing or too-short token falls back to an optional `default_slot_id`. `KemperRigNameCallback` strips the token from the displayed rig name by default.
...
### Emulator 2.4.10.17
- Features:
    - Added **EFFECT_STATE_DYNAMIC** to the web editor: exposed as plain parameters (`digit_position`, `default_slot_id`, `token_start`, `token_end`), no custom widget needed.

- Bug fixes:
    - Applying changes in the display editor now removes orphaned `DisplayLabel` constants from display.py that are no longer referenced in the Splashes tree.
